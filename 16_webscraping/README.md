...
**Prerequisites:** Module 3 — NB 12 (HTTP & APIs, JSON) is essential; NB 13 (SQL) is helpful. NB 7 (pandas) throughout. NB 25 (document processing / structured extraction) helps with the Firecrawl lesson.
...
```
                 ...
                 ┌───────────────┴───────────────────┐
                 ▼                                    ▼
      Is there an open API?  ── yes ─▶  Use it.  (NB 49 — OpenAlex)
                 │                       cleanest, most stable, most polite
                 no
                 ▼
      Is the page static HTML?  ── yes ─▶  Parse it yourself.
                 │                          (NB 47 — requests + BeautifulSoup)
                 no  (JavaScript-rendered / anti-bot / messy)
                 ▼
      Use a managed scraper.  ──────────▶  (NB 48 — Firecrawl → markdown / typed JSON)
```

Everything in this module runs **100% offline**: NB 47 parses an inline mock website, NB 48 uses a built-in `MockFirecrawl`, and NB 49 ships a canned OpenAlex snapshot that automatically upgrades to the live API when a network is present. No API key is ever required.

## Notebooks at a glance

| # | Notebook | ⏱ Time | Difficulty | What you'll build |
|---|---|---|---|---|
| 47 | `47_web_scraping_fundamentals.ipynb` | ~70 min | Intermediate | A polite, paginated scraper — BeautifulSoup, CSS selectors, tables → pandas, `robots.txt`, throttling & caching |
| 48 | `48_scraping_with_firecrawl.ipynb` | ~60 min | Intermediate | Managed scraping with Firecrawl — scrape/crawl/map/search, schema-driven **typed JSON** extraction |
| 49 | `49_openalex_scholarly_data.ipynb` | ~65 min | Intermediate | A scholarly-data pipeline on the **OpenAlex** open API — filtering, cursor pagination, a citations mini-analysis |

## Notebook guides

### 47 · Web Scraping Fundamentals — `47_web_scraping_fundamentals.ipynb`
...
### 48 · Scraping with Firecrawl — `48_scraping_with_firecrawl.ipynb`
...
- Choose deliberately between Firecrawl and the `requests`+BeautifulSoup approach of NB 47.
...
### 49 · OpenAlex: The Open Scholarly Graph — `49_openalex_scholarly_data.ipynb`
...
1. **Check for an API before you scrape.** A clean, documented endpoint beats a brittle HTML parser every time (NB 49).
