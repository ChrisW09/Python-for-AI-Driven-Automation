% ============================================================
% Lecture deck — Module 6 / Notebook 21
...
\subtitle{Module 6 --- AI Engineering $\cdot$ Notebook 21}
...
\begin{frame}{The training objective: next-token prediction}
  ...
  \vspace{2pt}
  {\footnotesize You write this exact loop --- loss $\to$ backward $\to$ step --- yourself in \textbf{Module 17 (PyTorch)}; an LLM trains with the same five moves, just with more parameters and more data.}
\end{frame}
...
\begin{frame}{Three levels of using an LLM}
  ...
  \begin{exampleblock}{Fine-tuning vs RAG}
    Fine-tuning changes \emph{behaviour / style}. \textbf{RAG} (NB 23) keeps the weights frozen and supplies \emph{fresh facts} --- don't fine-tune to add knowledge.
  \end{exampleblock}
\end{frame}
...
\begin{frame}{Two structural limitations}
  ...
  \begin{exampleblock}{Mitigations (built later in the course)}
    \textbf{RAG} (NB 23) grounds answers in evidence $\cdot$ \textbf{Tools} (NB 24) let it look things up $\cdot$ \textbf{Evaluation} (NB 26) measures how often it's wrong.
  \end{exampleblock}
\end{frame}
