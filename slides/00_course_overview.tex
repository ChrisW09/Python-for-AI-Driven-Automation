...
\begin{frame}{Sixteen modules, forty-six lessons (+ 13 appendices)}
  \scriptsize
  \begin{tabular}{rll}
    \toprule
...
\begin{frame}{When to pick which path}
  \begin{block}{Complete beginner}
    Never written code before. Take all modules in \textbf{spiral order} (see it work first, then the skills, then the builds). Time-budget: \textbf{about 115 hours} — the interactive estimator in \texttt{00b} computes yours.
  \end{block}
  \begin{exampleblock}{Analyst who knows Excel and SQL}
    Skim Module 1 (Python basics); Module 3's SQL will feel familiar. Focus on Modules 2 (data science), 4 (ML), 5 (industry applications), 13 (capstone). Time-budget: \textbf{about 43 hours}.
  \end{exampleblock}
  \begin{block}{ML practitioner adding AI engineering}
    Skip foundations and data science. Focus on Modules 6 (AI engineering), 8 (agents \& MCP), 11 (production), 13 (capstone). Time-budget: \textbf{about 34 hours}.
  \end{block}
\end{frame}
...
\begin{frame}{Plus two big capstones at the end}
  \begin{exampleblock}{Capstone A — AI Support-Bot Analytics (NB 41)}
  ...
  \end{exampleblock}
  \vspace{6pt}
  \begin{block}{Capstone B — AI Customer-Feedback Assistant (NB 42)}
...
\begin{frame}{LLM providers — local and hosted, four options}
  ...
  \begin{exampleblock}{See the dedicated guide}
    Notebook \texttt{06\_ai\_engineering/A1\_llm\_providers\_guide.ipynb} has the
