...
> 🧭  [◀ CI/CD & Deployment](../14_cicd/)  ·  [🏠 Course home](../README.md)  ·  [Business AI ▶](../16_business_ai/)

**Goal:** Apply everything from the technical track (NB 1–46) to ship two real-shape projects you can talk about in an interview or a code review.
...
**Prerequisites:** Capstone A: Notebooks 1–19 (Modules 1–5: Python, pandas, NumPy, visualization, statistics, scikit-learn). Capstone B: Notebooks 1–46 — especially AI engineering (NB 27–32) and production (NB 45–46).

```
                        The whole course (NB 1–46)
                                    │
             ┌──────────────────────┴──────────────────────┐
             ▼                                             ▼
  Capstone A — Analytics (NB 47)           Capstone B — AI Assistant (NB 48)
  fed by Modules 1–5:                      fed by Modules 8 & 13:
...
Like every notebook in the course, both capstones run **100% offline**. Capstone A needs only the standard analytics stack (pandas, NumPy, matplotlib/seaborn, scikit-learn) and generates its own data. Capstone B vendors a deterministic inline `MockLLM` — the twin of `MockLLM` from [`llm_providers.py`](../llm_providers.py) (NB 28) — so the whole assembly line runs with no internet and no API key, producing the same output every time; when you want real intelligence, one line swaps in OpenAI, Anthropic, Gemini, or a local Ollama model (see the [LLM Providers Guide](../08_ai_engineering/A1_llm_providers_guide.ipynb)). Unlike the regular lessons, the capstones aren't punctuated by quick-exercise checkpoints: each numbered section is a **milestone** that adds one piece to a single shippable artefact, and the 🧪 bonus and 🧠 stretch exercises at the end extend that artefact — try each one before expanding its collapsible 💡 solution.
...
Congratulations — with NB 1–46 plus these two capstones, the core technical track is complete. One module remains, the *practitioner* layer:
