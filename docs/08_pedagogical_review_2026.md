> **Note (July 2026).** This document predates the learning-order renumbering (modules 0–17 / lessons 1–52 were re-sequenced); lesson and module numbers below refer to the **old** numbering and are kept as a historical record. The scope and verification counts in the body (e.g. "63 notebooks", "9 fast-track", "all N parse") are what *this* review covered on its date and are left intact as a record.
...
| `18_ai_workflows` | Notebook **11** | Notebook 22 |
| `19_embeddings_retrieval` | Notebook **15** | Notebook 23 |
| `20_tools_and_agents` | Notebook **16** | Notebook 24 |
| `21_document_processing` | Notebook **17** | Notebook 25 |
| `23_from_notebook_to_project` | Notebook **18** (+ Module "Software Engineering") | Notebook 40 (Module: Production) |
| `24_scheduling_orchestration` | Notebook **19** (+ "Software Engineering") | Notebook 41 (Module: Production) |
| `26_capstone_analytics` | Notebook **10** | Notebook 43 |
...
- **Document processing (NB 25) → "From Notebook to Project"** — skipped AI Evaluation (NB 39). Now → **AI Evaluation (NB 39)**.

### 1.3 Two false "course is over" endings fixed
- **NB 41 (Scheduling)** ended with *"Congratulations — you've completed the 19-notebook journey… you've reached the end."* A complete-beginner following the course in order would have **stopped before the capstones**. Replaced with a proper Module 11 → Module 13 (Capstones) hand-off.
- **NB 27 (BPM/Governance)** declared *"you have completed the course"* with no mention of **Module 7** (NB 32–36). Reworded to point forward to Module 7 if not yet done.

### 1.4 New content added to close a genuine gap
- **Fast-track NB 3 (`03_lists_and_dicts`)** was titled and advertised as covering dictionaries, `defaultdict`, and `Counter`, but contained **zero dictionary content** — while fast-track NB 2 and NB 3's own exercises *use* dicts. Added a compact **"Dictionaries — a 2-minute primer"** (markdown + runnable cell: literals, `.get()`, `.items()`, `Counter`) so the fast-track is self-consistent.
...
- **Onboarding module table (NB 0 §4)** had broken markdown — rows 7 (Capstones) and 8 (Business AI) were missing their "What you'll build" cell, and row 9's description ("Two end-to-end projects to ship") actually belonged to the Capstones row. Table completed and corrected.
...
- **Stale dual reference in NB 26** ("NB 11 — legacy" *and* "NB 22 — canonical" for the same AI-workflows notebook) collapsed to a single correct pointer (NB 22).
- **`costkit` mis-attribution** ("NB 5's costkit_demo") reconciled to NB 40, where the package is actually built.
...
- **NB 22 (AI workflows)** — added a `💡` explaining *how the MockLLM decides what to return* (system-prompt keyword routing) and that every provider returns the same dict shape — the missing mental model behind why later "structured output" cells "magically" produce JSON.
...
- **NB 24 (tools & agents)** — two Stretch *solutions* monkey-patched a non-existent/no-op method (`MockLLM._should_call.__func__`, `_should_call_rules = lambda…`) that silently did nothing. Replaced with an honest note (the offline mock routes by keyword; a real provider routes from the tool description) plus a direct call so the cell actually demonstrates something.
- **Fast-track NB 8 & 09** — `from llm_providers import …` raised `ModuleNotFoundError` when run from the `fast_track/` folder (the module lives at the repo root). Added a 3-line `sys.path` bootstrap that walks up to find it. **Verified the import now succeeds from the fast-track folder.** Also fixed a broken back-link (`./A1_llm_providers_guide.ipynb` → `../06_ai_engineering/…`).
...
- **NB 00b time estimator** — the interactive estimator's `TIMES_MIN` table stopped at NB 44, silently scoring Modules 8–9 as 0 minutes. Added entries for NB 28–36 and a warning for any unknown notebook.
...
**Redundancy to consolidate (Modules 8–9 deliberately overlap Module 6)**
- **NB 26 ↔ NB 29** re-teach IDE choice, prompt patterns, and an identical "team prompt-style guide" Stretch exercise. Keep the split (seminar vs hands-on) but dedupe the duplicated stretch and signpost NB 29 as "applying" NB 26.
- **NB 17 §6 ↔ NB 18 §3** present nearly the same vector-store landscape table twice. Make NB 18 canonical; reduce NB 17 to a pointer.
- **NB 16** teaches the cost-optimal-threshold pattern three times (§5, Exercise 1, Stretch C); Stretch C could be repurposed. **NB 21**'s Stretch C duplicates the §5 cyclical-encoding demo.

**Content design**
- **NB 18 (agentic AI)** — the main ReAct POC teaches a brittle free-text/regex parser that the notebook's own Stretch solution calls the wrong approach; consider promoting the structured tool-calling API to the POC and keeping the regex version as an aside.
- **NB 23 / NB 25** — the inlined-vs-imported `MockLLM`/`MockEmbedder` duplication is worth a one-line "these are the same class" note.
- **NB 28 §6** — define `d_k` and the one-hot convention for the attention/cross-entropy formulas (the most intimidating page for a non-maths learner).
- **NB 30** — two stray German words ("Faustregel") should be translated; add a Module-4 prerequisite note before the XGBoost POC 3.
...
- **Notebook import robustness.** The canonical Module 6 notebooks use the same bare `from llm_providers import …` that broke the fast-track; they work under the test runner but may fail if a learner opens them directly. Consider the same `sys.path` bootstrap repo-wide.
...
The biggest find of the second pass. The AI notebooks import the shared helper with a bare `from llm_providers import …`, but `llm_providers.py` lives **only at the repo root**. JupyterLab starts a kernel with its working directory set to the *notebook's own folder*, and the project's own `run_all_notebooks.py` runs each notebook with `cwd = notebook's folder` too. Verified empirically: `from llm_providers import MockLLM` raises **`ModuleNotFoundError` from `06_ai_engineering/`**. So a learner opening NB 22–23 or appendices A1–A3 directly in JupyterLab would hit a hard import error on the first AI cell. (The recorded "pass" snapshot was produced with the repo root forced onto the path.)

**Fix:** added a 3-line, cwd-independent bootstrap (walks up from the working directory to find `llm_providers.py` and puts it on `sys.path`) to the five notebooks that import it — `18_ai_workflows`, `19_embeddings_retrieval`, `A1_llm_providers_guide`, `A2_vector_stores_survey`, `A3_rag_and_agent_frameworks` — the same fix already applied to the two fast-track AI notebooks. **Verified:** NB 22 now runs 18 cells cleanly *from its own folder*, and the import resolves from a subfolder cwd. (NB 45 only *mentions* `llm_providers` in prose — no change needed.)

### 7.2 Remaining stale cross-references (missed by pass 1)
- **NB 12 (matplotlib)** — two more "the NB 10 capstone" references (cells 0 and 62) → **NB 43** (pass 1 fixed only the other two).
- **NB 15 (sklearn)** — "The LLM-based approach in **NB 11**" → **NB 22** (old layout had AI-workflows at 11).
- **Fast-track `09_embeddings_and_rag`** — five references to "Notebook 11"/"NB 11" for the keyword-RAG (which is fast-track notebook 8), plus a prerequisite line citing canonical `NB 5 / 11 / 15` (and NumPy, which the fast track skips) → corrected to fast-track numbers.
- **Fast-track NB 4 & NB 7** — inline "NB 11 / NB 15" canonical references that don't exist in the fast track → reworded.
...
- **NB 43** module label "Capstone Project" → "Capstones" (matches NB 44 and the module name).
- **Fast-track titles** unified to "Notebook N (fast track) — …" across all nine.

### 7.4 Content improvements
- **NB 28** — named the *one-hot* target in the cross-entropy formula and added a one-line definition of `d_k` under the attention table (the lone undefined symbol on the most intimidating page).
- **NB 30** — translated the stray German "**Faustregel:**" → "**Rule of thumb:**".
...
- Executed 14 notebooks end-to-end with the available libraries (foundations NB 2–6, NB 7, NB 10–12, NB 24, NB 22, fast-track 03/04/08) — **all clean**, including the previously-broken fast-track and AI-notebook imports. (DS/ML notebooks needing scikit-learn/statsmodels couldn't be executed in the sandbox for lack of disk space; their edited cells were verified by syntax check and, for the behaviour-sensitive ones, by standalone equivalence tests.)
- Confirmed every edit is persisted on disk in the course folder.

### 7.6 Items still open after this pass
Genuinely-future refinements (unchanged from §5, minus what was completed above): Module 14↔5 / 9↔5 overlap consolidation (NB 26↔33, NB 17↔36); NB 18's brittle free-text ReAct parser vs structured tool-calling; and the inline-vs-imported `MockLLM`/`MockEmbedder` duplication note in NB 23/21. None blocks a learner. *(The Module 2 ↔ 3 ordering and the time-estimate reconciliation were addressed in §8.)*

---

## 8. Structural restructuring — the motivation-first spiral (pass 3)

**Diagnosis.** The course built skills strictly bottom-up but deferred *both* motivating elements to the very end: the **why** (Module 14, business context) and the **payoff** (Module 7, actually building an AI POC). A learner spent ~17 notebooks on Python/data/ML before anything looked like "AI-driven automation." The dependency order is correct and can't be scrambled (pandas needs Python, RAG needs embeddings, the POCs need the whole stack), so the fix is a **spiral**: pull the prerequisite-free framing and a working demo to the front, pull professional tooling forward, keep the skill spine in order, and cluster the build + deployment judgement at the end.
...
- **Spiral guidance in the master onboarding (NB 0)** — a new "🧭 The recommended order" section at the top: (1) see it work (`00c`) + optionally the *why* (NB 45) and *what an LLM is* (NB 28); (2) build skills Modules 1→6 with **Git/Copilot (NB 26) set up early**; (3) build the real thing (Module 7 POCs + Module 14 deployment judgement); (4) ship a capstone (Module 13). The environment-check success message and the "which file next" section now point to `00c` first.
...
- **Orientation signposts** (additive 📍 notes) on NB 45 ("recommended early — prerequisite-free"), NB 26 ("do this early — the Git/Copilot workflow pays off everywhere"), and NB 28 ("concepts are prerequisite-free; skim the maths first time"). Plus "🧭 Where this fits" tie-backs in the Foundations and Data-Science module READMEs to sustain the through-line during the long skill-building stretch.
...
| **NB 23** | bonus `rag_answer_safe` — `KeyError: 'tech.mobile'` | Stretch C/D reassigned the global `DOCS` to a smaller corpus the retriever wasn't built on | renamed the exercises' local corpus to `KB` |
| **NB 23** | Debug-me solution — `NameError: new_doc_text` | solution wasn't self-contained | define `new_doc_text` in the block |
| **NB 24** | bonus `run_pandas_query` — `'dict' has no attribute 'append'` | Stretch exercises reassigned `TOOLS` (a schema *list*) to a name→function *dict* | renamed the exercises' dict to `TOOL_MAP` |
| **NB 25** | Stretch solution — `'function' has no attribute '__func__'` | `MockLLM._extract` is a `@staticmethod`; `.__func__` is wrong | call `MockLLM._extract(text)` directly |
| **NB 44** | two Stretch solutions — `NameError: send_slack_alert`; `FileNotFoundError` | a helper was never defined; a report dir was never created | add an offline `send_slack_alert` stub; `out_dir.mkdir(exist_ok=True)` |
| **NB 8** | bonus CTE+JOIN — `no such table: channel_meta` | a later cell reset `conn` to a fresh in-memory DB, dropping the lookup table | re-register `channel_meta` on the current connection |
| **fast-track 09** | same Debug-me `NameError` as NB 23 | trimmed copy carried the same gap | same self-containment fix |
...
- **NB 2 Debug-me** infinite loop — *intentional* puzzle code, with an explicit "don't run this" warning; the real solution is correct.
- **NB 7** bonus `KeyError: 'city'` — the checker ran the `safe_fetch` *stub*; with the real implementation it works online (this is a live-API bonus).
- **NB 40** `SyntaxError` — an illustrative `...`-elided CLI fragment, not standalone code.
- **NB 18** `ModuleNotFoundError: faiss` — an optional-dependency demo, by design.
...
- **NB 26 ↔ NB 29 overlap (signposted + de-duplicated).** On inspection the two are mostly *complementary*, not duplicated — NB 26 §3 teaches *general* code-prompting patterns; NB 29 §5 teaches *app-scaffolding* prompt structure (stack, files, schemas, demo data). The genuine duplication was the near-identical "team prompt-style guide" stretch in both. Fixes: NB 29 §5 now back-references NB 26 §3 ("there you learned *what* makes a good prompt; here you apply it"); NB 29's Stretch B is reframed as an *Agent-Mode addendum* that builds on NB 26's guide rather than repeating it; and NB 26 §6 now points forward to Module 7 as where the workflow is *practised* ("this notebook is the judgement; Module 7 is the practice").
- **NB 18 ReAct parser (contradiction resolved).** The POC 2 prompt claimed a regex parser of `Thought/Action/Action Input` is "reliable," while the notebook's own Debug-me solution correctly calls free-text regex parsing "brittle" and names structured tool-calling as the fix. The POC bullet now states this honestly: regex *can* drive the loop, but the model deviates in practice, so **production agents use the structured tool-calling API from §8** — the free-text version is kept only because it makes every *Thought*/*Action* visible, which is the point of a teaching POC. This reconciles §8, the POC, and the Debug-me lesson.
- **NB 23 / NB 25 inline-vs-imported mocks (clarified).** Both vendor a tiny `MockLLM` inline while other notebooks import it from `llm_providers.py`. Added a one-line note in each explaining the inline copy is "the same idea as `llm_providers.py`'s (NB 22), vendored for self-containment" — and fixed a stale `from NB 11` comment in NB 23 (the mock pattern comes from NB 22, not 11).
- **NB 17 ↔ NB 18 vector-store landscape (already fine — left as-is).** Re-checked: NB 17 §6 is a brief, use-case-mapped 5-bullet list that already says "*(full landscape covered in NB 18)*", and NB 18 §3 is the canonical 6-row table + selection heuristic. That is a sensible split (pick-a-store-now vs the full comparison), not a true duplication, so it was left unchanged.

**Verification (open-items pass):** reference checker green; 64 notebooks parse, 0 syntax errors; the five edited notebooks (NB 23, 21, 30, 33, 36) all execute cleanly; edits confirmed on disk.
...
- **Gaps closed**: the placeholders at NB 9 (Pydantic — folded into the SQL notebook) and NB 42 (config & secrets — folded into NB 39) no longer leave holes; every number 1–34 exists.
...
| 9, 25 | — | gaps eliminated (content folded into NB 13 / NB 39) |
...
Independent module-by-module sweeps (onboarding+foundations, data science, I/O+ML, AI engineering, production+capstones, business-AI+POCs, fast-track+quizzes+requirements) found the rest of the course correct: all 30 quiz answer keys verified correct; NB 26's Transformer math (attention, √dₖ scaling, cross-entropy) and NB 29's cosine-similarity worked examples check out; no stale cross-references; no placeholders, TODOs, or untranslated leftovers; capstones self-contained and top-to-bottom runnable. Remaining Module 14↔9 overlaps are the intentional seminar-vs-hands-on split already signposted in §10.
