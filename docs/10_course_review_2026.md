...
3. **DeepTab real backend.** NB38 runs via the offline stand-in; if a learner installs `deeptab`, the real Mambular path has further 1.8.0 API drift (`.encoder`, proba shape) — worth a one-time alignment if real-backend execution becomes a goal.
4. **Provider pricing/IDs** in `A1_llm_providers_guide` for OpenAI/Google are caveated but unverified — a periodic refresh against each provider's docs is the only durable fix (they drift).
