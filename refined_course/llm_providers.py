...
class MockLLM:
    ...
    def chat(self, messages, *, temperature=0.0, max_tokens=512, **_):
        ...
        if "json" in sys_msg and ("sentiment" in sys_msg or "topic" in sys_msg):
            ...
            out = shorten(re.split(r"(?<=[.!?])\s+", user_msg.strip(), maxsplit=1)[0],
                           width=120, placeholder="…")
        elif "answer" in sys_msg:
            out = ("Based on the provided context, here is a short answer. "
                   f"You asked: {shorten(user_msg, 80, placeholder='…')}")
        else:
        ...
        return {"text": out, "model": "mock-mini",
