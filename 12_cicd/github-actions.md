...
- `working-directory:` changes into the backend folder *for this step only*, so `requirements.txt` is found without long paths. It expands `APP_DIR` to `12_cicd/example-app/backend`.
...
jobs:
  lint:
    ...
    steps:
      ...
      - name: Run ruff
        working-directory: 12_cicd/example-app/backend
        run: ruff check .
```
