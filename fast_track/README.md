...
- **The optional appendices** (classical → deep-learning → foundation-model forecasting, PyTorch, TabPFN/conformal prediction, vector-store & RAG/agent-framework surveys, the pricing / A/B-testing / credit-risk / causal-uplift industry deep dives, the Module 7 business-function tour — HR, finance, sales, procurement — its growth & operations set — marketing mix, service operations, routing, insurance, product analytics, bandits — and the DuckDB/Polars big-data tour), **Module 12 (DeepTab)**, **Module 16 (Business AI)** as a standalone lesson, and **Module 17 (Django)**.
